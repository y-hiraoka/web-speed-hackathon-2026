...
// Generate search page HTML (search form)
function generateSearchHtml(): string {
  const inputClass = "flex-1 rounded border px-4 py-2 focus:outline-none border-cax-border focus:border-cax-brand-strong";
  const buttonClass = "flex items-center justify-center gap-2 rounded-full border px-4 py-2 disabled:cursor-not-allowed disabled:opacity-50 bg-cax-brand text-cax-surface-raised hover:bg-cax-brand-strong border-transparent";

  return `<div class="flex flex-col gap-4"><div class="bg-cax-surface p-4 shadow"><form><div class="flex gap-2"><div class="flex flex-1 flex-col"><input class="${inputClass}" placeholder="検索 (例: キーワード since:2025-01-01 until:2025-12-31)" type="text" value=""></div><button class="${buttonClass}" type="submit"><span>検索</span></button></div></form><p class="text-cax-text-muted mt-2 text-xs">since:YYYY-MM-DD で開始日、until:YYYY-MM-DD で終了日を指定できます</p></div></div>`;
}
...
ssrRouter.get("/{*splat}", async (req, res, next) => {
  ...
  try {
    ...
    if ((url === "/terms" || url === "/search") && req.session.userId == null) {
    ...
    // For /dm, /crok, /users, etc. - just render the shell with no page content
